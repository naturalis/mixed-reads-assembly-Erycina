...
for SAMPLE in $SAMPLES; do
	...
	if [ ! -e $SAM.filtered ]; then
		...
		# -F REMOVED should keep all reads
		# -q 20 = remove reads with mapping qual < 20
		# 
		echo "going to run samtools view -bS -q 20 $SAM > $SAM.filtered"
		samtools view -bS -q 20 $SAM > $SAM.filtered
		gzip -9 $SAM
	else
